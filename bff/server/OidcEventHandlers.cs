...
public static class OidcEventHandlers
{
    ...
    {
        var logoutUri = $"https://{configuration.GetValue<string>("Auth0Domain")}/v2/logout?client_id={configuration.GetValue<string>("Auth0ClientId")}";
    ...
    private static async Task OnPushAuthorizationHandler(PushedAuthorizationContext context, IConfiguration configuration)
    {
        ...
        context.ProtocolMessage.Parameters.Add("audience", configuration.GetValue<string>("Auth0Audience"));
